...
: m_seed(0),
  ...
  m_algorithm(QSettings().value("Algorithm", 1).toInt()) {
	...
	restart_icon.addPixmap(QPixmap(":/16x16/view-refresh.png"));
        QAction* restart_action = game_menu->addAction(restart_icon, tr("&Restart"), this, SLOT(restartGame()), tr("F5"));
#ifndef Q_OS_SYMBIAN
...
//	help_menu->addAction(tr("About &Qt"), qApp, SLOT(aboutQt()));
    ...
    connect( actAbout, SIGNAL(triggered(bool)), SLOT(about()) );
...
//#ifdef Q_OS_SYMBIAN
        ...
        QAction* undo_action =  new QAction(undo_icon,tr("&Undo"), toolbar);
        connect(undo_action, SIGNAL(triggered()), moves, SLOT(undo()));
       // QAction* undo_action = toolbar->addAction(undo_icon, tr("&Undo"), moves, SLOT(undo()), tr("Ctrl+z"));
        undo_action->setEnabled(false);
        connect(moves, SIGNAL(canUndoChanged(bool)), undo_action, SLOT(setEnabled(bool)));
        ...
        QAction* redo_action =  new QAction(redo_icon,tr("&Redo"), toolbar);
        connect(redo_action, SIGNAL(triggered()), moves, SLOT(redo()));

        //QAction* redo_action = toolbar->addAction(redo_icon, tr("&Redo"), moves, SLOT(redo()), tr("Shift+Ctrl+Z"));
        redo_action->setEnabled(false);
        connect(moves, SIGNAL(canRedoChanged(bool)), redo_action, SLOT(setEnabled(bool)));
//#endif
...
void Window::about() {
	QMessageBox::about(this, tr("About Peg-E"), tr(
		"<center>"
                "<big><b>Peg-E for Mobile %1</b></big><br/>"
		"Peg elimination game<br/>"
	    "Peg-e is licenced under GPLv3<br/>"
			 "<small>Ported to BlackBerry 10 by Antti Pohjola, antti.pohjola@gmail.com</small><br/>"
			 "<small>you can get the source code for this app at:</small><br/>"
			 "<small>https://github.com/Summeli/BlackBerry-10-Port-A-Thon</small><br/>"
             "<small>Ported to Symbian by QtK</small><br/>"
		"<small>Original Game 2009 Graeme Gott</small><br/><br/>"
